...
public class AllPartyHallAdpater extends RecyclerView.Adapter<AllPartyHallAdpater.ViewHolder> {

    private final int limit = 5;
    Context mContext;
    ArrayList<PartyHallModel.Party> partyArrayList = new ArrayList<>();

    public AllPartyHallAdpater(Context mContext, ArrayList<PartyHallModel.Party> partyArrayList) {
    ...
    @NonNull
    @Override
    public AllPartyHallAdpater.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(mContext).inflate(R.layout.see_all_banquets_item, parent, false);
        return new ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull AllPartyHallAdpater.ViewHolder holder, int position) {
